...
def registerPage(request):
    ...
    #pass the user data
    if request.method == 'POST':
        form = MyUserCreationForm(request.POST) #through in UCform
        if form.is_valid():
...
def home(request):
    ...
    room_messages=Message.objects.filter(Q(room__name__icontains=q))[0:3]
...
def userProfile(request,pk):
    ...
    # can get the childs of all specific obj
...
def createRoom(request):
    ...
    if request.method == 'POST':
        ...
        topic, created = Topic.objects.get_or_create(name=topic_name)
         #g_or_c()-will return a obj,if new one found and not in db it will create it
...
def updateRoom(request,pk):
    ...
    if request.method == 'POST':
        topic_name = request.POST.get('topic')
        topic, created = Topic.objects.get_or_create(name=topic_name)
        room.name = request.POST.get('name') #for update
        room.topic = topic
...
def updateUser(request):
    user = request.user
    form = UserForm(instance=user) # Pre-fill form with book's data
    if request.method == 'POST':
